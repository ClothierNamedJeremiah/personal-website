...
describe('Portfolio', () => {
  it('renders all projects', async () => {
    ...
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent("Some Things I've Built");
    ...
    projectShowcases.forEach((projectShowcase, index) => {
      const { title, description, sourceCodeUrl, liveSiteUrl } = projects[index];

      expect(getByRole(projectShowcase, 'heading', { level: 2 })).toHaveTextContent(
        'Project Showcase',
      );

      expect(getByRole(projectShowcase, 'heading', { level: 3 })).toHaveTextContent(title);
